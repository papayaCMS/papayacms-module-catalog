...
* @version $Id: content_catalog_unsubscribe.php 39732 2014-04-08 15:34:45Z weinert $
*/
...
class content_catalog_unsubscribe extends base_content {
  ...
  */
   ...
   *
   * @access public
   * @internal param string $action rem
   */
  ...
  */
  function checkUnsubscription() {
    $valid = TRUE;
    if (!PapayaFilterFactory::isEmail($this->params['email']) || $this->params['email'] == '') {
  ...
  */
  function getConfirmString() {
    ...
    $res = $this->catalog->databaseQueryFmt(
      $sql,
      array($this->tableCatalogSubscriptionsMeta)
    );
    if ($res) {
      foreach ($res as $row) {
  ...
  function processConfirmation($confirmStr, $email) {
    ...
    if (!empty($confirmStr) && $email != '') {
      if (isset($this->params['catalog_ids']) &&
        ...
        $this->_messages[] = array(MSG_INFO, $this->data['nothing_selected']);
        $success = FALSE;
      }
    }
    return $success;
  ...
  */
  function loadSubscribedCatalogs($confirmStr) {
