...
* @version $Id: cronjob_catalog_subscriptions.php 39818 2014-05-13 13:15:13Z weinert $
*/
  ...
  */
  function initializeDialog() {
    ...
    if (isset($this->params['cmd'])) {
      ...
      case 'addlng':
        return $this->getContentForm($this->data, $this->papaya()->administrationLanguage->id);
        break;
      case 'store':
        ...
        $this->data['content'][$this->papaya()->administrationLanguage->id] = array(
          'subject' => $this->params['subject'],
          'body' => $this->params['body'],
        );
        $this->data['hostname'][$this->papaya()->administrationLanguage->id] =
  ...
  */
  function getDialog() {
    ...
    if (isset($this->data['content']) && is_array($this->data['content'])
        ...
        && isset($this->data['content'][$languageId])
        || isset($this->params['cmd']) && $this->params['cmd'] == 'addlng') {
      $this->getContentForm($this->data, $languageId);
  ...
  function getContentForm($data = NULL, $lngId = NULL) {
    if (!isset($this->dialog) || !is_object($this->dialog)) {
      ...
      $this->dialog->dialogTitle =
        $this->_gt('Edit Content').': '.$this->papaya()->administrationLanguage->id;
