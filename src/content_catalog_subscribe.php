...
* @version $Id: content_catalog_subscribe.php 39732 2014-04-08 15:34:45Z weinert $
*/
...
class content_catalog_subscribe extends base_content {
  ...
  */
   ...
   * @var string
   */
  public $tableCatalogSubscriptionsPending = '';

  /**
   * @var string
   */
  public $tableCatalogSubscriptionsMeta = '';

  /**
   * @var string
   */
  public $tableCatalogSubscriptions = '';

  /**
   * @var array|NULL
   */
  public $catalogRelated = NULL;
  ...
  */
   ...
   *
   * @internal param string $action add
   * @access public
   */
  function processSubscription() {
    if ($confirmString = $this->getConfirmString()) {
      if ($subscriptionId = $this->getSubscriptionId()) {
        ...
        if (
          $this->catalog->databaseInsertRecord(
  ...
  */
  function checkSubscription() {
    $valid = TRUE;
    if (!PapayaFilterFactory::isEmail(@$this->params['email'], TRUE)) {
      $this->_messages[] = array(MSG_ERROR, $this->data['invalid_email']);
      $valid = FALSE;
    }
    if (!is_array(@$this->params['catalog_ids']) ||
        count($this->params['catalog_ids']) < 1) {
      $this->_messages[] = array(MSG_ERROR, $this->data['nothing_selected']);
      $valid = FALSE;
    }
    if (is_array(@$this->params['catalog_ids']) &&
        ...
        if ($data = $res->fetchRow(DB_FETCHMODE_ASSOC)) {
          ...
          if ($res = $this->catalog->databaseQueryFmt($sql, $params)) {
            ...
            if (isset($ids) && is_array($ids) && count($ids) > 0) {
              ...
              if (
                $this->catalog->databaseInsertRecords($this->tableCatalogSubscriptions, $values)
              ) {
              ...
              $this->_messages[] = array(MSG_INFO, $this->data['nothing_new']);
            }
            $condition = array('catalog_subscription_confirmstring' => $cfStr);
            if (
              !$this->catalog->databaseDeleteRecord(
          ...
          $this->_messages[] = array(MSG_INFO, $this->data['no_pending']);
        }
      } else {
        $this->_messages[] = array(MSG_INFO, $this->data['no_pending']);
      }
  ...
  */
  function loadRelatedCatalogs($topicId, $lngId) {
