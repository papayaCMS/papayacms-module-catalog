...
* @version $Id: base_catalog.php 39732 2014-04-08 15:34:45Z weinert $
*/
/**
* Catalog base module
*
...
class base_catalog extends base_db {
  ...
  */
   ...
   * @var array|NULL
   */
  public $categoriesWithSynonyms = NULL;

  /**
   * @var array|NULL
   */
  public $catalogSynonymList = NULL;

  /**
   * @var array|NULL
   */
  public $catalogSynonyms = NULL;

  /**
   * @var array
   */
  public $topicTeaser = NULL;
    ...
    if ($catalogId > 0 && isset($this->catalogs[$catalogId])) {
      ...
      if (!isset($catalog['newpath'])) {
        if ($catalog['catalog_parent'] > 0) {
          if (isset($this->catalogs[$catalog['catalog_parent']])) {
  ...
  */
  function getXMLCategoryList($catalogId, $lngId) {
    ...
    if (isset($this->catalogList) && is_array($this->catalogList)) {
      ...
      $categoryKeys = PapayaUtilArrayMapper::byIndex($categories, 'catalog_id');
        ...
        if (isset($linkCount[$category['catalog_id']][$lngId])) {
          ...
          if (isset($this->linkList[$category['catalog_id']]) &&
            ...
            foreach ($this->linkList[$category['catalog_id']] as $link) {
              $result .= sprintf(
                '<link href="%s" title="%s" chargroup="%s"/>'.LF,
                papaya_strings::escapeHtmlChars(
                  $this->getWebLink(
                    $link['topic_id'],
                    '',
  ...
  */
  function getImageResizedTeaser($xml, $width, $height, $resize) {
    ...
    if (trim($xml) != '') {
      ...
      $dom->loadXML($xml);
      $nodes = $dom->xpath()->evaluate(
        '//subtopic[@no]/image/[name() = "papaya:media"]/@src'
      );
      $thumbs = array();
      foreach ($nodes as $sourceAttribute) {
        $number = $dom->xpath()->evaluate('number(ancestor::subtopic[1]/@no)');
        $thumbs[(int)$number] = sprintf(
          '<papaya:media src="%s" width="%d" height="%d" resize="%s" />',
          papaya_strings::escapeHtmlChars($sourceAttribute->value),
          (int)$width,
          (int)$height,
  ...
  */
  function getFirstLetterNormalized($str) {
    $letter = papaya_strings::normalizeString($str);
    $letter = papaya_strings::substr($letter, 0, 1);
  ...
  * @param base_topic $topicObj topic object
  * @access public
  * @return array $result
  */
  ...
  */
  public function getTopicTitles($languageId, $categId = 0, $topicIds = NULL) {
    $pagesConnector = $this->papaya()->plugins->get(
