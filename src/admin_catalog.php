...
* @version $Id: admin_catalog.php 39818 2014-05-13 13:15:13Z weinert $
*/
...
class admin_catalog extends base_catalog {
  ...
  */
   ...
   * @var base_dialog
   */
  private $dialogCatalog = NULL;

  /**
   * @var base_dialog
   */
  private $catalogTypeDialog = NULL;

  /**
   * @var array
   */
  private $sessionPageParams;
      ...
      case 'save_synonym':
        if (isset($this->params['catalog_id']) && $this->params['catalog_id'] != '') {
          if (isset($this->params['catalogsynonym_id']) &&
            ...
            if ($this->databaseUpdateRecord($this->tableCatalogSynonyms, $data, $condition)) {
              $this->addMsg(
                MSG_INFO,
                sprintf(
  ...
  */
  function getXMLClipboard() {
    if (isset($this->clipboard) && is_array($this->clipboard)) {
      ...
      $result .= '<items>'.LF;
      foreach ($this->clipboard as $value) {
  ...
  */
  function getXMLInfobar() {
    ...
    foreach ($this->papaya()->languages as $language) {
      if ($language['image'] != '' &&
          file_exists($this->getBasePath(TRUE).'pics/language/'.$language['image'])) {
        $image = ' image="./pics/language/'.
          papaya_strings::escapeHtmlChars($language['image']).'"';
      } else {
        $image = '';
      }
      $result .= sprintf(
      ...
      if (isset($this->catalogDetails) &&
          isset($this->catalogDetails[$language['id']]) &&
          is_array($this->catalogDetails[$language['id']]) ) {
        if (isset($this->catalogDetails[$language['id']]['catalog_title'])) {
          $result .= sprintf(
            '<listitem title="%s" indent="2"><subitem>%s</subitem></listitem>'.LF,
            papaya_strings::escapeHtmlChars($this->_gt('Title')),
            papaya_strings::escapeHtmlChars(
              $this->catalogDetails[$language['id']]['catalog_title']
            )
          );
        } else {
          $result .= sprintf(
  ...
  */
   ...
   * @return string $result XML
   */
  function getXMLCatalogEntry($id, $indent) {
    $result = '';
  ...
  */
  ...
  function getXMLSubscriptions() {
    ...
    if ($res = $this->databaseQueryFmt($sql, $params, $limit, $offset)) {
      while ($row = $res->fetchRow(DB_FETCHMODE_ASSOC)) {
        if (PapayaFilterFactory::isEmail($row['catalog_subscription_email'], TRUE)) {
          $resultSet[] = $row;
        }
      }
    }
  ...
  function getXMLSubscriptionEdit() {
    if (isset($this->params['catalog_subscription_email'])
      ...
      $result .= '</items></listview>'.LF;
      $this->getDeleteSubscriptionsForm();
  ...
  */
  function getXMLSynonymList() {
    ...
    $this->loadCatalogLinkList(
      $catalogId, $this->papaya()->administrationLanguage->id, 0, FALSE
    );
      ...
      foreach ($this->linkList[$catalogId] as $data) {
        if (isset($data['topic_id'])) {
          ...
          $result .= sprintf(
            ...
            papaya_strings::escapeHtmlChars($title),
            papaya_strings::escapeHTMLChars($images[$imgIndex]),
            $selected
          );
  ...
  function initializeTopicEditForm() {
    if (!is_object($this->_topicEditForm)) {
      ...
      foreach ($this->papaya()->languages as $languageId => $language) {
        ...
        if (isset($this->topicDetailLngGroups[$languageId])) {
          foreach ($this->topicDetailLngGroups[$languageId] as $catalogLinkId => $title) {
        ...
        $fields['cataloglink_titles_new_'.$languageId] = array(
          'Add',
          'isNoHTML',
          FALSE,
          'input',
  ...
  */
  function loadCatalogCounts() {
    if (isset($this->catalogsOpen) &&
        is_array($this->catalogsOpen) &&
        isset($this->catalogs) &&
        is_array($this->catalogs) &&
        count($this->catalogs) > 0) {
      $ids = array_keys($this->catalogs);
      $filter = $this->databaseGetSqlCondition('catalog_parent', $ids);
    } else {
      $filter = '1=1';
    }
